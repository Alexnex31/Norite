...
FROM message_tags t
JOIN messages m ON m.id = $2::bigint
  AND m.channel_id = $3::bigint AND m.deleted_at IS NULL
...
const countMessageTagApplicationsByOthers = `-- name: CountMessageTagApplicationsByOthers :one
SELECT count(*) FROM message_tag_applications WHERE tag_id = $1 AND applied_by <> $2
`

type CountMessageTagApplicationsByOthersParams struct {
	TagID     int64
	AppliedBy int64
}
...
// leading tag_id.
func (q *Queries) CountMessageTagApplicationsByOthers(ctx context.Context, arg CountMessageTagApplicationsByOthersParams) (int64, error) {
	row := q.db.QueryRow(ctx, countMessageTagApplicationsByOthers, arg.TagID, arg.AppliedBy)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
const lockMessageTag = `-- name: LockMessageTag :exec
...
func (q *Queries) LockMessageTag(ctx context.Context, id int64) error {
	_, err := q.db.Exec(ctx, lockMessageTag, id)
	return err
}
