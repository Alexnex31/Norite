...
-- Serves the directory's default sort. Partial on discoverable, because the rows it excludes are the
...
GET    /guilds/discover?sort=&after=&limit= -- M72a; the public directory. Reads only discoverable=true,
